...
const Recorder = () => {
    ...
    async function startStream(data, id, options, permissions, permissions2) {
        ...
        if (helperVideoStream.current.getAudioTracks().length > 0) {
            ...
            audioOutputSource.current = aCtx.current.createMediaStreamSource(helperVideoStream.current);
            audioOutputSource.current.connect(audioOutputGain.current).connect(destination.current);
        } else {
    ...
    return (
        <div className="wrap">
            <img className="logo" src={chrome.runtime.getURL('assets/logo-text.svg')} />
