...
const PopupContainer = (props) => {
    ...
    useEffect(() => {
        ...
        if (!locale.includes('en')) {
            setURL(`https://translate.google.com/translate?sl=en&tl=${locale}&u=https://help.screenity.io/`);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (tab === 'record') {
            ...
            pillRef.current.style.width = recordTabRef.current.getBoundingClientRect().width + 'px';
        } else {
            pillRef.current.style.left = videoTabRef.current.offsetLeft + 'px';

            pillRef.current.style.width = videoTabRef.current.getBoundingClientRect().width + 'px';
        }
    ...
    const handleDrag = (e, d) => {
        ...
        if (d.x - 40 < width || d.x > window.innerWidth + 10 || d.y < 0 || d.y + height + 40 > window.innerHeight) {
                        ...
                        {!onboarding && (
                            <Tabs.Root className="TabsRoot tl" defaultValue="record" onValueChange={onValueChange}>
                                <Tabs.List className="TabsList tl" data-value={tab} aria-label="Manage your account" tabIndex={0}>
                                    ...
                                    <Tabs.Trigger className="TabsTrigger tl" value="dashboard" ref={videoTabRef} tabIndex={0}>
                                        <div className="TabsTriggerIcon">
                                            <img src={tab === 'dashboard' ? VideoTabActive : VideoTabInactive} />
                                        </div>
